...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".swiftcoder/glossary/zh-cn.md"
  if (locale === "zht") return ".swiftcoder/glossary/zh-tw.md"
  return `.swiftcoder/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("SwiftCoder did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("SwiftCoder returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("SwiftCoder session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel SwiftCoder runs for 'all' (default: 4)
      --model <provider/id>  SwiftCoder model (default: opencode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running SwiftCoder
...
`)
  ...
  if (failed.length) console.error(`\nSwiftCoder failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.SWIFTCODER_DISABLE_PROJECT_CONFIG = "1"
  env.SWIFTCODER_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["swiftcoder", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.SWIFTCODER_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["swiftcoder", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.SWIFTCODER_CONFIG
  delete env.SWIFTCODER_CONFIG_DIR
  delete env.SWIFTCODER_CONFIG_CONTENT
  delete env.SWIFTCODER_PERMISSION
  delete env.SWIFTCODER_AUTO_SHARE
  return env
}
