...
- Author package entrypoint is `@swiftscale/plugin/tui`.
...
```json
{
  "$schema": "https://swift-scale.com/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@swiftscale/plugin/tui`.
- `@swiftscale/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@swiftscale/plugin/tui"
...
- The value is a semver range checked against the running SwiftCoder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.swiftcoder` is absent, no check is performed (backward compatible).
...
- `swiftcoder plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `swiftcoder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.swiftcoder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.swiftcoder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.swiftcoder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.swiftcoder` dir or `SWIFTCODER_CONFIG_DIR`, SwiftCoder installs `@swiftscale/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@swiftscale/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus SwiftCoder's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
SwiftCoder registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@swiftscale/plugin/tui`.
...
- `message` is required; `title` defaults to `"swiftcoder"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `swiftcoder.default`.
...
- Local plugins persist installed themes under the local `.swiftcoder/themes` area near the plugin config source.
...
- `--pure` / `SWIFTCODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.swiftcoder/plugins/tui-smoke.tsx`
- Local vim plugin: `.swiftcoder/plugins/tui-vim.tsx`
- Local smoke config: `.swiftcoder/tui.json`
- Local smoke theme: `.swiftcoder/plugins/smoke-theme.json`
