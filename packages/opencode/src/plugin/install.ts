...
export type PatchDeps = {
  ...
  files: (dir: string, name: "swiftcoder" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".swiftcoder")
}

function patchName(kind: Kind): "swiftcoder" | "tui" {
  if (kind === "server") return "swiftcoder"
  return "tui"
}
