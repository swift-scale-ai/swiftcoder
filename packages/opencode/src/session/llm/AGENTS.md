...
- `native-request.ts` converts swiftcoder's normalized session input into a native `@opencode-ai/llm` `LLMRequest`. It does not execute requests.
...
src/session/
  ...
  llm/
    ...
    native-request.ts       swiftcoder/AI SDK-shaped input -> @opencode-ai/llm LLMRequest
