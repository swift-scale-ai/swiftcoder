...
const USER_AGENT = `swiftcoder/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const params = yield* input.plugin.trigger(
    ...
    },
    {
      ...
      maxOutputTokens: ProviderTransform.maxOutputTokens(
        input.model,
        input.user.model.outputTokenMax ?? input.flags.outputTokenMax,
      ),
      options,
    },
  )
  ...
  const opencodeProjectID = input.model.providerID.startsWith("swiftcoder")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("swiftcoder")
        ? {
            ...(opencodeProjectID ? { "x-swiftcoder-project": opencodeProjectID } : {}),
            "x-swiftcoder-session": input.sessionID,
            "x-swiftcoder-request": input.user.id,
            "x-swiftcoder-client": input.flags.client,
