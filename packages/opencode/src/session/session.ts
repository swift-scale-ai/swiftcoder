...
import { Slug } from "@swiftscale/core/util/slug"
...
import type { ProviderMetadata, Usage } from "@swiftscale/llm"
...
import { SessionV2 } from "@swiftscale/core/session"
import * as SessionExecutionLocal from "@swiftscale/core/session/execution/local"
import { locationServiceMapLayer } from "@swiftscale/core/location-services"
...
import { MessageTable, PartTable, SessionTable } from "@swiftscale/core/session/sql"
import { ProjectTable } from "@swiftscale/core/project/sql"
import { MessageV2 } from "./message-v2"
import type { InstanceContext } from "../project/instance-context"
import { InstanceState } from "@/effect/instance-state"
import { Snapshot } from "@/snapshot"
import { ProjectV2 } from "@swiftscale/core/project"
import { WorkspaceV2 } from "@swiftscale/core/workspace"
import { SessionID, MessageID, PartID } from "./schema"

import type { Provider } from "@/provider/provider"
import { Global } from "@swiftscale/core/global"
import { Effect, Layer, Option, Context, Schema, Types } from "effect"
import { NonNegativeInt, optional } from "@swiftscale/core/schema"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { ProviderV2 } from "@swiftscale/core/provider"
import { ModelV2 } from "@swiftscale/core/model"
import { SessionMessage } from "@swiftscale/schema/session-message"
...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".swiftcoder", "plans")
...
export interface Interface {
  ...
  readonly reconcileInterrupted: () => Effect.Effect<number>
}

export class Service extends Context.Service<Service, Interface>()("@swiftcoder/Session") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const reconcileInterrupted = Effect.fn("Session.reconcileInterrupted")(function* () {
      const ctx = yield* InstanceState.context
      const rows = yield* db
        .select({ id: MessageTable.id, sessionID: MessageTable.session_id, info: MessageTable.data })
        .from(MessageTable)
      ...
      for (const row of rows) {
        const info = { ...row.info, id: row.id, sessionID: row.sessionID } as SessionV1.Info
        if (info.role !== "assistant" || info.time.completed || info.error) continue
        ...
        for (const row of parts) {
          const part = { ...row.data, id: row.id, messageID: row.message_id, sessionID: row.session_id } as SessionV1.Part
          ...
          yield* updatePart({
            ...
            state: {
              status: "error",
              error: "Tool execution interrupted by application restart",
        ...
        yield* updateMessage({
          ...info,
          time: { ...info.time, completed: end },
          error: new SessionV1.AbortedError({ message: "Task interrupted by application restart" }).toObject(),
        })
        count++
      }
      return count
