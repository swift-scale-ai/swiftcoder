...
import { IdeEvent } from "@swiftscale/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["SWIFTCODER_CALLER"] === "vscode" || process.env["SWIFTCODER_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.swiftcoder"], {
    nothrow: true,
  })
