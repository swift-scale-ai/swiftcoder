...
const AGENT_USERNAME = "swiftcoder-agent[bot]"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent - https://swift-scale.com/docs/github/#usage-examples",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/swiftcoder-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.swift-scale.com/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  swiftcoder:
    ...
    steps:
      ...
      - name: Run swiftcoder
        uses: swiftscale/swiftcoder/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.swift-scale.com" : "https://swift-scale.com"
    ...
    try {
      ...
      console.log("swiftcoder session", session.id)
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.swift-scale.com"
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to swiftcoder...")
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `swiftcoder/${type}-${hex}-${timestamp}`
      }
      return `swiftcoder/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      ...
      const shareUrl = shareId ? `[swiftcoder session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the swiftcoder infrastructure after your response",
