// Shared type vocabulary for the direct interactive mode (`swiftcoder --mini`).
//
...
import type { SwiftCoderClient, PermissionRequest, QuestionRequest, ToolPart } from "@swiftscale/sdk/v2"
import type { TuiConfig } from "@swiftscale/coder-tui/config"
...
type PromptModel = Parameters<SwiftCoderClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<SwiftCoderClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<SwiftCoderClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<SwiftCoderClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<SwiftCoderClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<SwiftCoderClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<SwiftCoderClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<SwiftCoderClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<SwiftCoderClient["question"]["reject"]>[0]
