import type { PermissionV1 } from "@swiftscale/core/v1/permission"
import { FSUtil } from "@swiftscale/core/fs-util"
// CLI entry point for `swiftcoder run` and `swiftcoder --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`swiftcoder --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`swiftcoder --mini --attach`): connects to a running
//      swiftcoder server and runs interactive mode against it.
//
...
import { createSwiftCoderClient, type SwiftCoderClient, type ToolPart } from "@swiftscale/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<SwiftCoderClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run swiftcoder with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running swiftcoder server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to SWIFTCODER_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to SWIFTCODER_SERVER_USERNAME or 'swiftcoder')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: SwiftCoderClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: SwiftCoderClient, sessionID: string) {
      ...
      async function current(sdk: SwiftCoderClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: SwiftCoderClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: SwiftCoderClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: SwiftCoderClient) {
        ...
        async function loop(client: SwiftCoderClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createSwiftCoderClient({
