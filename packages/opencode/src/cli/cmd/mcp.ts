...
import { ConfigMCPV1 } from "@swiftscale/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: swiftcoder mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in swiftcoder.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .swiftcoder/ subdirectory too)
  const candidates = [path.join(baseDir, "swiftcoder.json"), path.join(baseDir, "swiftcoder.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".swiftcoder", "swiftcoder.json"), path.join(baseDir, ".swiftcoder", "swiftcoder.jsonc"))
  }
  ...
  // Default to swiftcoder.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., swiftcoder x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
