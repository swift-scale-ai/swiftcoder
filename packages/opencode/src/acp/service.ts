...
import type { AssistantMessage, Message, SwiftCoderClient, SessionMessageResponse } from "@opencode-ai/sdk/v2"
...
export class Service extends Context.Service<Service, Interface>()("@swiftcoder/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `swiftcoder auth login` in the terminal",
      name: "Login with swiftcoder",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: SwiftCoderClient) {
...
function makeUsageService(sdk: SwiftCoderClient) {
...
async function loadDirectorySnapshot(sdk: SwiftCoderClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, swiftcoder provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const opencodeProvider = providers[ProviderV2.ID.make("swiftcoder")]
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "SwiftCoder prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "SwiftCoder service failure", service })
}
