...
import { isRecord } from "@swiftscale/coder-tui/util/record"
...
import { TuiKeybind } from "@swiftscale/coder-tui/config/keybind"
...
import { TuiConfig } from "@swiftscale/coder-tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@swiftcoder/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old swiftcoder.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.swiftcoder`
  // folders between cwd and home, and SWIFTCODER_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SWIFTCODER_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit SWIFTCODER_TUI_CONFIG override, if set.
  if (Flag.SWIFTCODER_TUI_CONFIG) {
    const configFile = Flag.SWIFTCODER_TUI_CONFIG
  ...
  // 4. `.swiftcoder` directories (and SWIFTCODER_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".swiftcoder") || dir === Flag.SWIFTCODER_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".swiftcoder") && dir !== Flag.SWIFTCODER_CONFIG_DIR) continue
