...
const TUI_SCHEMA_URL = "https://swift-scale.com/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from swiftcoder.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const swiftcoder = await opencodeFiles(input)
  for (const file of swiftcoder) {
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "swiftcoder"),
    ...(await Filesystem.findUp(["swiftcoder.json", "swiftcoder.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "swiftcoder"))
  }
  if (Flag.SWIFTCODER_CONFIG) files.push(Flag.SWIFTCODER_CONFIG)
