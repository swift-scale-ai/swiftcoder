...
import { NonNegativeInt, PositiveInt } from "@swiftscale/core/schema"
import { ConfigAttachmentV1 } from "@swiftscale/core/v1/config/attachment"
import { ConfigLSPV1 } from "@swiftscale/core/v1/config/lsp"
import { InvalidError } from "@swiftscale/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by SwiftCoder V1. Use V1 "permission" rules or run opencode2.',
      })),
    })
