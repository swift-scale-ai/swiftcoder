...
import type * as SDK from "@swiftscale/sdk/v2"
...
import { SessionShareTable } from "@swiftscale/core/share/sql"
...
const disabled = process.env["SWIFTCODER_DISABLE_SHARE"] === "true" || process.env["SWIFTCODER_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@swiftcoder/ShareNext") {}
  ...
  Effect.gen(function* () {
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        const baseUrl = (yield* cfg.get()).enterprise?.url ?? "https://swift-scale.com"
