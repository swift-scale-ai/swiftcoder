...
import { SwiftCoderHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for swiftcoder serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(SwiftCoderHttpApi)
...
export const PublicApi = SwiftCoderHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "swiftcoder",
    version: "1.0.0",
    description: "swiftcoder api",
