...
import { Hash } from "@swiftscale/core/util/hash"
...
import { optional } from "@swiftscale/core/schema"
...
const isSwiftScaleProductModel = (id: string) =>
  isSwiftScaleTextModelID(id) &&
...
export function applySwiftScaleCatalog(models: Record<string, Model>, modelIDs?: string[]) {
  const allowed = new Set(
    (modelIDs ?? [SWIFTSCALE_FALLBACK_MODEL]).filter((id) => {
      if (isSwiftScaleProductModel(id)) return true
      return isSwiftScaleDirectModel(id) && models[id] !== undefined
    }),
  )

  const template = models[SWIFTSCALE_FALLBACK_MODEL] ?? Object.values(models)[0]
  if (template) {
    for (const id of allowed) {
      if (!isSwiftScaleProductModel(id)) continue
      if (models[id]) continue
      models[id] = {
        ...template,
        id: ModelV2.ID.make(id),
        api: { ...template.api, id },
        family: id.replace(/\.auto$/, ""),
      }
    }
  }
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@swiftscale/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
  ...
  return {
    ...
    swiftcoder: Effect.fnUntraced(function* (input: Info) {
      ...
      if (ok && key) {
        ...
        const baseURL = resolveSwiftScaleBaseURL({
          configured: typeof configuredURL === "string" ? configuredURL : undefined,
      ...
      return {
        ...
        options: {
          ...(ok ? {} : { apiKey: "public" }),
          baseURL: resolveSwiftScaleBaseURL({
            configured:
              typeof cfg.provider?.["swiftcoder"]?.options?.baseURL === "string"
                ? cfg.provider["swiftcoder"].options.baseURL
                : undefined,
            environment: env.SWIFTCODER_GATEWAY_BASE_URL,
            catalog: Object.values(input.models)[0]?.api.url,
          }),
          headerTimeout: OPENAI_HEADER_TIMEOUT_DEFAULT,
          chunkTimeout: SWIFTSCALE_STREAM_IDLE_TIMEOUT,
          ...(key
            ? {
                fetch: createSwiftScaleAuthenticatedFetch({
                  initialAccessToken: key,
          ...
          headers: {
            ...(codingPlan
              ? {
                  "X-SwiftScale-Usage-Mode": "interactive-agent",
                  "X-SwiftScale-Agent-Session": SWIFTSCALE_AGENT_SESSION,
                }
              : {}),
          },
        },
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from swiftcoder.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://swift-scale.com/",
            "X-Title": "swiftcoder",
            "X-Source": "swiftcoder",
          },
        },
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://swift-scale.com/",
            "X-Title": "swiftcoder",
            "X-BILLING-INVOKE-ORIGIN": "SwiftCoder",
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `swiftcoder/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `swiftcoder/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `swiftcoder auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `swiftcoder/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "swiftcoder",
          },
        },
      }),
    kilo: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, swiftcoder auth, or provider options.`,
            )
          },
        }
      }
...
export class Service extends Context.Service<Service, Interface>()("@swiftcoder/Provider") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [id, provider] of Object.entries(auths)) {
          ...
          if (providerID === ProviderV2.ID.swiftcoder && provider.type === "oauth") {
            mergeProvider(providerID, {
              source: "api",
              key: provider.access,
            })
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = providerID.startsWith("swiftcoder")
        ? ["gpt-nano"]
