...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  const item = providers[ProviderV2.ID.make("swiftcoder")]
...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    yield* remove("SWIFTCODER_API_KEY")
    const error = yield* Provider.use
      .getModel(ProviderV2.ID.swiftcoder, ModelV2.ID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance(
  ...
  {
    config: {
      provider: { "cloudflare-ai-gateway": { options: { metadata: { invoked_by: "test", project: "swiftcoder" } } } },
    },
  },
)
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".swiftcoder")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".swiftcoder")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
...
test("SwiftScale catalog keeps entitled Swift, GPT, Claude, and Gemini models", () => {
  const models = {
    "swiftlite.auto": { name: "SwiftScale" },
    "swiftpro.auto": { name: "Swift Pro" },
...
test("SwiftScale catalog does not invent unknown direct model metadata", () => {
  ...
  applySwiftScaleCatalog(models, ["gpt-future", "claude-future", "gemini-future"])

  expect(Object.keys(models)).toEqual([])
})

test("Coding Plan catalog exposes one SwiftScale model", () => {
  const models = {
    "swiftlite.auto": { name: "Swift Lite" },
    "swiftpro.auto": { name: "Swift Pro" },
  } as any
...
test("SwiftScale catalog gives discovered product models distinct names", () => {
  const models = {
    "swiftlite.auto": { name: "SwiftScale", api: { id: "swiftlite.auto" } },
  } as any

  applySwiftScaleCatalog(models, ["swiftlite.auto", "swiftcustom.auto", "swiftlong-context.auto"])
...
it.effect("swiftcoder loader falls back to the Coding Plan model when catalog lookup fails", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped({
      config: { provider: { swiftcoder: { options: { apiKey: "test-key" } } } },
    })
    ...
    expect(Object.keys(none[ProviderV2.ID.swiftcoder].models)).toEqual(["swiftlite.auto"])
    expect(Object.keys(keyed[ProviderV2.ID.swiftcoder].models)).toEqual(["swiftlite.auto"])
  }).pipe(provideMultiInstance),
)

it.effect("swiftcoder loader does not expose legacy models when auth catalog lookup fails", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped()
    ...
    yield* Effect.acquireRelease(
      Effect.promise(() =>
        Filesystem.write(authPath, JSON.stringify({ swiftcoder: { type: "api", key: "test-key" } })),
      ),
      () =>
        Effect.promise(async () => {
          if (original !== undefined) await Filesystem.write(authPath, original)
