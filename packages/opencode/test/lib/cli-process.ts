// Subprocess test harness for the swiftcoder CLI. Spawns the real binary against
...
//
// Configuration flows through swiftcoder's built-in test affordances:
//   - SWIFTCODER_CONFIG_CONTENT      : provider config inline, no files to find
//   - SWIFTCODER_TEST_HOME           : pins os.homedir() → tmpdir
//   - SWIFTCODER_DISABLE_PROJECT_CONFIG : skip walking up for swiftcoder.json
//   - SWIFTCODER_PURE                : skip external plugin discovery + install
//   - SWIFTCODER_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `swiftcoder.run` is fully wired. The shape supports adding more
// builders (`swiftcoder.serve(opts)`, `swiftcoder.acp(opts)`, `swiftcoder.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of SwiftCoderCli.
...
import { AppProcess } from "@swiftscale/core/process"
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    SWIFTCODER_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    SWIFTCODER_CONFIG_CONTENT: configJson,
    SWIFTCODER_DISABLE_PROJECT_CONFIG: "1",
    SWIFTCODER_PURE: "1",
    SWIFTCODER_DISABLE_AUTOUPDATE: "1",
    SWIFTCODER_DISABLE_AUTOCOMPACT: "1",
    SWIFTCODER_DISABLE_MODELS_FETCH: "1",
...
// Typed equivalent of constructing argv for `swiftcoder run`. New flags should
...
// `swiftcoder serve` is a long-lived process — it never exits on its own.
...
// `swiftcoder acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type SwiftCoderCli = {
  ...
  // Spawn `swiftcoder serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `swiftcoder acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly swiftcoder: SwiftCoderCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("swiftcoder.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          SWIFTCODER_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("swiftcoder.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("swiftcoder.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "swiftcoder server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `swiftcoder serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("swiftcoder.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const swiftcoder: SwiftCoderCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, swiftcoder })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `swiftcoder.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "swiftcoder") {
...
// every call site. Use this for any test that needs the swiftcoder CLI fixture.
//
...
// resources (e.g. `swiftcoder.serve`) without an extra `Effect.scoped` wrapper —
