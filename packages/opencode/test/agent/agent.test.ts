...
it.instance("plan agent denies edits except .swiftcoder/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".swiftcoder/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      // Unconfigured mutations retain SwiftCoder's approval default.
      expect(evalPerm(build, "edit")).toBe("ask")
    }),
  {
    config: {
...
it.instance("default permission asks before mutations, commands, network, and external access", () =>
  Effect.gen(function* () {
    const build = yield* load((svc) => svc.get("build"))
    expect(evalPerm(build, "edit")).toBe("ask")
    expect(evalPerm(build, "bash")).toBe("ask")
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".swiftcoder", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.SWIFTCODER_TEST_HOME
      process.env.SWIFTCODER_TEST_HOME = test.directory
