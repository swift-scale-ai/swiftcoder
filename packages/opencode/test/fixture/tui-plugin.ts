...
import type { HostPluginApi } from "@swiftscale/coder-tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createSwiftCoderClient({
  ...
  let selected = opts.theme?.selected ?? "swiftcoder"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "swiftcoder.default",
