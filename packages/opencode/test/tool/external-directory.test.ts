...
describe("tool.assertExternalDirectory", () => {
  ...
  it.instance("asks when a project symlink points outside the instance directory", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const outerTmp = yield* tmpdirScoped()
      const link = path.join(test.directory, "linked-outside")
      const target = path.join(link, "nested", "file.txt")
      const fsUtil = yield* FSUtil.Service
      const canonicalTarget = yield* fsUtil.resolve(path.join(outerTmp, "nested", "file.txt"))
      const expected = glob(path.join(path.dirname(target), "*"))
      const { requests, ctx } = makeCtx()

      yield* Effect.promise(() => fs.symlink(outerTmp, link, "dir"))
      ...
      expect(req!.metadata.filepath).toBe(canonicalTarget)
    }),
  )
