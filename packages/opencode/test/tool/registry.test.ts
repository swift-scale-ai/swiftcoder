...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".swiftcoder")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .swiftcoder/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const swiftcoder = path.join(test.directory, ".swiftcoder")
      const tool = path.join(swiftcoder, "tool")
  ...
  it.instance("ignores non-tool exports in .swiftcoder/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".swiftcoder", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-swiftcoder` was registering a tool
  ...
  it.instance("loads tools from .swiftcoder/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const swiftcoder = path.join(test.directory, ".swiftcoder")
      const tools = path.join(swiftcoder, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".swiftcoder", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const swiftcoder = path.join(test.directory, ".swiftcoder")
        const customTools = path.join(swiftcoder, "tools")
        const plugin = path.join(swiftcoder, "node_modules", "@opencode-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(swiftcoder, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@swiftscale/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@swiftscale/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".swiftcoder", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(swiftcoder, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@swiftscale/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(swiftcoder, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(swiftcoder, "node_modules", "cowsay")
