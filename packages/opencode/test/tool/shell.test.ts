...
describe("tool.shell", () => {
  ...
  each("captures output from immediately exiting commands", () =>
    runIn(
      projectRoot,
      Effect.gen(function* () {
        for (let i = 0; i < 10; i++) {
          const result = yield* run({ command: `printf fast-${i}; printf err-${i} >&2` })
          expect(result.metadata.exit).toBe(0)
          expect(result.metadata.output).toContain(`fast-${i}`)
          expect(result.metadata.output).toContain(`err-${i}`)
        }
      }),
    ),
  )
    ...
    for (const item of ps) {
      it.live(`asks for external_directory permission for missing PowerShell env paths [${item.label}]`, () =>
        withShell(
          ...
          Effect.acquireUseRelease(
            Effect.sync(() => {
              const key = "SWIFTCODER_TEST_MISSING"
