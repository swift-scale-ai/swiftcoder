...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/swiftcoder.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/swiftcoder.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/swiftcoder.jsonc", dir: ".swiftcoder", suggestion: "swiftcoder" },
        expected:
          'Directory ".swiftcoder" in /tmp/swiftcoder.jsonc is not valid. Rename the directory to "swiftcoder" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/swiftcoder.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/swiftcoder.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.swift-scale.com/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.swift-scale.com/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `swiftcoder models` to list available models",
      "Or check your config (swiftcoder.json) provider/model names",
    ].join("\n")
