// Subprocess integration tests for `swiftcoder run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `swiftcoder.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `SWIFTCODER_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("swiftcoder run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* swiftcoder.run("say hi")
        swiftcoder.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* swiftcoder.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* swiftcoder.run("think", { extraArgs: ["--thinking"] })
        swiftcoder.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* swiftcoder.run("think again")
        swiftcoder.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* swiftcoder.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* swiftcoder.run("say hi", { format: "json" })
        swiftcoder.expectExit(result, 0)

        const events = swiftcoder.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const result = yield* swiftcoder.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* swiftcoder.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* swiftcoder.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* swiftcoder.run("request permission", { permission: { bash: "ask" } })
        swiftcoder.expectExit(denied, 0)
        ...
        const allowed = yield* swiftcoder.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        swiftcoder.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* swiftcoder.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        swiftcoder.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, swiftcoder }) =>
      Effect.gen(function* () {
        ...
        const server = yield* swiftcoder.serve()

        const result = yield* swiftcoder.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, swiftcoder }) =>
      Effect.gen(function* () {
        const result = yield* swiftcoder.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, swiftcoder }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* swiftcoder.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
