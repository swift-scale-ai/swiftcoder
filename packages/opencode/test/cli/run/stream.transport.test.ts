...
type EventStream = Awaited<ReturnType<SwiftCoderClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<SwiftCoderClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<SwiftCoderClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<SwiftCoderClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<SwiftCoderClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: SwiftCoderClient["event"]["subscribe"]
    globalEvent?: SwiftCoderClient["global"]["event"]
    promptAsync?: SwiftCoderClient["session"]["promptAsync"]
    status?: SwiftCoderClient["session"]["status"]
    messages?: SwiftCoderClient["session"]["messages"]
    children?: SwiftCoderClient["session"]["children"]
    permissions?: SwiftCoderClient["permission"]["list"]
    questions?: SwiftCoderClient["question"]["list"]
  } = {},
) {
  const client = new SwiftCoderClient()

  const subscribe: SwiftCoderClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: SwiftCoderClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: SwiftCoderClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: SwiftCoderClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: SwiftCoderClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: SwiftCoderClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: SwiftCoderClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: SwiftCoderClient["question"]["list"] = input.questions ?? (() => ok([]))
