...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ swiftcoder }) =>
    Effect.gen(function* () {
      const result = yield* swiftcoder.spawn(["--replay-limit", "10"])

      swiftcoder.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ swiftcoder }) =>
    Effect.gen(function* () {
      const result = yield* swiftcoder.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ swiftcoder }) =>
    Effect.gen(function* () {
      const result = yield* swiftcoder.spawn(["--mini", "--port", "4096"])
