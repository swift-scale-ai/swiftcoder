...
// terminal sizes. The default swiftcoder tui command is excluded —
// `swiftcoder --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `swiftcoder --help` prints today;
...
// top-level help on `--help` and exits 1; not a real swiftcoder command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("swiftcoder CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const topLevel = yield* swiftcoder.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* swiftcoder.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`swiftcoder ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`swiftcoder ${argv.join(" ")} --help`)
        }
