...
describe("swiftcoder acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ swiftcoder }))
        ...
        expect(initialized.agentInfo?.name).toBe("SwiftCoder")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ swiftcoder })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.SWIFTCODER_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
