...
describe("swiftcoder read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // SWIFTCODER_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const r = yield* swiftcoder.spawn(["mcp", "list"])
        swiftcoder.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const r = yield* swiftcoder.spawn(["providers", "list"])
        swiftcoder.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const r = yield* swiftcoder.spawn(["models"])
        swiftcoder.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const r = yield* swiftcoder.spawn(["agent", "list"])
        swiftcoder.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh SWIFTCODER_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const r = yield* swiftcoder.spawn(["session", "list"])
        swiftcoder.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const r = yield* swiftcoder.spawn(["stats"])
        swiftcoder.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under SWIFTCODER_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ swiftcoder }) =>
      Effect.gen(function* () {
        const r = yield* swiftcoder.spawn(["db", "path"])
        swiftcoder.expectExit(r, 0, "db path")
