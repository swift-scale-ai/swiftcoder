...
import { HttpRecorder } from "@swiftscale/http-recorder"
import { HttpRecorderInternal } from "@swiftscale/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@swiftscale/llm"
import { RequestExecutor } from "@swiftscale/llm/route"
...
import { LayerNodePlatform } from "@swiftscale/core/effect/app-node-platform"

const FIXTURES_DIR = path.join(import.meta.dir, "../fixtures/recordings")

const zenURL = (connection: string) => `https://console.swift-scale.com/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.SWIFTCODER_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["swiftcoder", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("SWIFTCODER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("SWIFTCODER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["swiftcoder", "native", "oauth", "tool-loop"],
  ...
  {
    id: "opencode-proxy",
    name: "SwiftCoder proxy",
    providerID: ProviderV2.ID.swiftcoder,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["swiftcoder", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.SWIFTCODER_RECORD_CONSOLE_TOKEN && process.env.SWIFTCODER_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.swiftcoder,
        name: "SwiftCoder",
        env: ["SWIFTCODER_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.SWIFTCODER_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.SWIFTCODER_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.SWIFTCODER_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["swiftcoder", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("SWIFTCODER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("SWIFTCODER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("SWIFTCODER_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set SWIFTCODER_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the swiftcoder LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "swiftcoder.json"),
      JSON.stringify({ $schema: "https://swift-scale.com/config.json", ...scenario.config(model) }),
    ),
  )
