...
import { SessionMessageTable } from "@swiftscale/core/session/sql"
...
import { Shell } from "@swiftscale/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@swiftscale/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "swiftcoder.json"),
    JSON.stringify({ $schema: "https://swift-scale.com/config.json", ...config }),
  )
})
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("swiftcoder"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
