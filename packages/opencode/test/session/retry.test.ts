...
import type { NamedError } from "@swiftscale/core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to SwiftCoder upgrade action", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "swiftcoder")).toEqual({
      message: SessionRetry.USAGE_UPGRADE_MESSAGE,
      action: {
        reason: "free_tier_limit",
        provider: "swiftcoder",
        title: "Free limit reached",
        message: "Subscribe to SwiftCoder for reliable access to the best open-source models, starting at $5/month.",
        label: "subscribe",
        link: SessionRetry.USAGE_UPGRADE_URL,
      },
    })
  })

  test("maps subscription limits to workspace usage settings", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "swiftcoder")).toEqual({
      message:
        "5 hour usage limit reached. It will reset in 5 hours 23 minutes. To continue using this model now, enable usage from your available balance - https://swift-scale.com/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      action: {
        ...
        label: "open settings",
        link: "https://swift-scale.com/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      },
    })
  })
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "swiftcoder")?.action?.message).toBe(
