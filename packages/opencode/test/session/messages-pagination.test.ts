...
describe("MessageV2.page", () => {
  ...
  it.instance(
    "pages through a 2,000-message session within the Phase 3 budget",
    () =>
      withSession(({ sessionID }) =>
        Effect.gen(function* () {
          ...
          const ids = yield* fill(sessionID, messageCount)

          const started = performance.now()
          const received = [] as MessageID[]
          let before: string | undefined
          do {
            const page = yield* MessageV2.page({ sessionID, limit: pageSize, before })
            received.unshift(...page.items.map((item) => item.info.id))
            before = page.cursor
          } while (before)
          ...
          console.log(`Phase 3 long-session pagination: ${durationMs.toFixed(1)} ms / ${budgetMs} ms`)
        }),
      ),
    { timeout: 60_000 },
  )
