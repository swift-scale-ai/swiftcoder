...
import { LLMEvent, ToolFailure } from "@swiftscale/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@swiftscale/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("swiftcoder"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("swiftcoder") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("swiftcoder") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, swiftcoder, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored swiftcoder auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("swiftcoder") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("swiftcoder"),
