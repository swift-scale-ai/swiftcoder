...
import { SessionTable } from "@swiftscale/core/session/sql"
...
import { WorkspaceTable } from "@swiftscale/core/control-plane/workspace.sql"
...
const originalEnv = {
  SWIFTCODER_AUTH_CONTENT: process.env.SWIFTCODER_AUTH_CONTENT,
  SWIFTCODER_EXPERIMENTAL_WORKSPACES: process.env.SWIFTCODER_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.SWIFTCODER_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.SWIFTCODER_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.SWIFTCODER_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.SWIFTCODER_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.SWIFTCODER_EXPERIMENTAL_WORKSPACES).toBe("true")
