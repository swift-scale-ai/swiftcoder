...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "swiftcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "swiftcoder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "swiftcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "swiftcoder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["swiftcoder.json", "swiftcoder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "swiftcoder.json"),
        path.join(project, "swiftcoder.jsonc"),
      ])
    })
  })
