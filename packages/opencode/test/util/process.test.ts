...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.SWIFTCODER_TEST ?? "")'), {
      env: {
        SWIFTCODER_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "SWIFTCODER_TEST_SHELL"], {
      shell: true,
      env: {
        SWIFTCODER_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("SWIFTCODER_TEST_SHELL=ok")
  })
