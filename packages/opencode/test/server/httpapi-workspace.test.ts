...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-swiftcoder-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-swiftcoder-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://swift-scale.com/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-swiftcoder-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-swiftcoder-workspace")
