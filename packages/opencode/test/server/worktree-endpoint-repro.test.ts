...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      SWIFTCODER_EXPERIMENTAL_WORKSPACES: Flag.SWIFTCODER_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.SWIFTCODER_EXPERIMENTAL_WORKSPACES = original.SWIFTCODER_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
