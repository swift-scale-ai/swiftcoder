...
Kit and Aidan want provider-specific LLM behavior to move out of swiftcoder's AI
...
swiftcoder's dynamic runtime should construct executable models at its app
...
- [ ] Update tests so direct route/provider tests assert route values are carried
      by executable models, and swiftcoder/native tests assert boundary-based route
      selection.
