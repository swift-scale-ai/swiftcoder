...
describe("SessionRunnerModel", () => {
  ...
  it.effect("marks Coding Plan requests as interactive agent traffic", () =>
    Effect.gen(function* () {
      const resolved = yield* SessionRunnerModel.fromCatalogModel(
        ModelV2.Info.make({
          ...model({ type: "aisdk", package: "@ai-sdk/openai", url: "https://api.swift-scale.com/v1" }),
          request: { headers: {}, body: {} },
        }),
        Credential.OAuth.make({
          type: "oauth",
          ...
          refresh: "refresh",
          expires: Date.now() + 60_000,
          metadata: { swiftScaleUsageMode: "coding_plan" },
        }),
      )

      expect(resolved.route.defaults.headers?.["X-SwiftScale-Usage-Mode"]).toBe("interactive-agent")
      expect(resolved.route.defaults.headers?.["X-SwiftScale-Agent-Session"]).toMatch(/^swiftcoder-/)
    }),
  )

  it.effect("keeps API Services routing metadata out of request bodies", () =>
    Effect.gen(function* () {
      const resolved = yield* SessionRunnerModel.fromCatalogModel(
        ModelV2.Info.make({
          ...model({ type: "aisdk", package: "@ai-sdk/openai", url: "https://api.swift-scale.com/v1" }),
      ...
      expect(resolved.route.defaults.http?.body).toEqual({})
      expect(resolved.route.defaults.headers?.["X-SwiftScale-Usage-Mode"]).toBeUndefined()
    }),
  )
