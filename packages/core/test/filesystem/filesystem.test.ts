...
describe("FSUtil", () => {
  describe("resolve", () => {
    it(
      "resolves non-existent paths through the nearest existing symlink ancestor",
      Effect.gen(function* () {
        const fs = yield* FSUtil.Service
        const filesys = yield* FileSystem.FileSystem
        const tmp = yield* filesys.makeTempDirectoryScoped()
        const outside = path.join(tmp, "outside")
        const link = path.join(tmp, "inside-link")
        yield* filesys.makeDirectory(outside)
        yield* Effect.promise(() => nodeFs.symlink(outside, link, "dir"))

        const target = path.join(link, "new", "file.txt")
        const canonicalOutside = yield* fs.resolve(outside)
        expect(yield* fs.resolve(target)).toBe(path.join(canonicalOutside, "new", "file.txt"))
      }),
    )
  })
