...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.SWIFTCODER_MODELS_PATH,
          disabled: Flag.SWIFTCODER_DISABLE_MODELS_FETCH,
        }
        Flag.SWIFTCODER_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.SWIFTCODER_MODELS_PATH = previous.path
          Flag.SWIFTCODER_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
