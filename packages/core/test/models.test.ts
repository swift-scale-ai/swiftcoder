...
// test/preload.ts pins SWIFTCODER_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.SWIFTCODER_MODELS_PATH
const ORIGINAL_MODELS_URL = Flag.SWIFTCODER_MODELS_URL
const ORIGINAL_DISABLE_FETCH = Flag.SWIFTCODER_DISABLE_MODELS_FETCH
const TEST_MODELS_URL = "https://catalog.test"
beforeAll(() => {
  Flag.SWIFTCODER_MODELS_PATH = undefined
  Flag.SWIFTCODER_MODELS_URL = TEST_MODELS_URL
  Flag.SWIFTCODER_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.SWIFTCODER_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.SWIFTCODER_MODELS_URL = ORIGINAL_MODELS_URL
  Flag.SWIFTCODER_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})

const cacheFile = path.join(Global.Path.cache, `models-${Hash.fast(TEST_MODELS_URL)}.json`)
...
describe("ModelsDev Service", () => {
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.SWIFTCODER_DISABLE_MODELS_FETCH = false
        }),
  ...
  it.live("does not fetch when no remote catalog URL is configured", () =>
    Effect.gen(function* () {
      const state = yield* Ref.make(initialState)
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.SWIFTCODER_MODELS_URL = undefined
          Flag.SWIFTCODER_DISABLE_MODELS_FETCH = false
        }),
        () =>
          Effect.gen(function* () {
            ...
            yield* svc.refresh(true)
            return yield* svc.get()
          }).pipe(Effect.provide(buildLayer(state))),
        () =>
          Effect.sync(() => {
            Flag.SWIFTCODER_MODELS_URL = TEST_MODELS_URL
            Flag.SWIFTCODER_DISABLE_MODELS_FETCH = true
