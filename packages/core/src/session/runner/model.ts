...
const swiftScaleAgentSession = `swiftcoder-${randomUUID()}`
const swiftScaleUsageModeKey = "swiftScaleUsageMode"
...
export class Service extends Context.Service<Service, Interface>()("@swiftcoder/v2/SessionRunnerModel") {}
...
export const fromCatalogModel = (
  ...
  const usageMode = credential?.metadata?.[swiftScaleUsageModeKey]
  const resolved =
    credential?.type !== "key" || credential.metadata === undefined
      ...
      : produce(model, (draft) => {
          Object.assign(
            draft.request.body,
            Object.fromEntries(
              Object.entries(credential.metadata ?? {}).filter(([key]) => key !== swiftScaleUsageModeKey),
            ),
          )
        })
  const routed =
    usageMode === "coding_plan"
      ? produce(resolved, (draft) => {
          draft.request.headers["X-SwiftScale-Usage-Mode"] = "interactive-agent"
          draft.request.headers["X-SwiftScale-Agent-Session"] = swiftScaleAgentSession
        })
      : resolved
  const key = apiKey(routed, credential)
  if (routed.api.type === "aisdk" && routed.api.package === "@ai-sdk/openai") {
    return Effect.succeed(
  ...
  if (routed.api.type === "aisdk" && routed.api.package === "@ai-sdk/anthropic") {
    return Effect.succeed(
      withDefaults(routed, AnthropicMessages.route)
        .with({ auth: key === undefined ? Auth.none : Auth.header("x-api-key", key) })
        .model({ id: routed.api.id }),
    )
  }
  if (routed.api.type === "aisdk" && routed.api.package === "@ai-sdk/openai-compatible" && routed.api.url) {
    return Effect.succeed(
      withDefaults(routed, OpenAICompatibleChat.route)
        .with({ auth: key === undefined ? Auth.none : Auth.bearer(key) })
        .model({ id: routed.api.id }),
    )
  }
