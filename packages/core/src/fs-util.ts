...
import { basename as pathBasename, dirname, isAbsolute, join, relative, resolve as pathResolve, sep } from "path"
...
export namespace FSUtil {
  ...
  export class Service extends Context.Service<Service, Interface>()("@swiftcoder/FileSystem") {}
  ...
  const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const resolve = Effect.fn("FileSystem.resolve")(function* (path: string) {
        ...
        const suffix: string[] = []
        let anchor = resolved
        while (!(yield* fs.exists(anchor).pipe(Effect.orElseSucceed(() => false)))) {
          const parent = dirname(anchor)
          if (parent === anchor) return resolved
          suffix.unshift(pathBasename(anchor))
          anchor = parent
        }
        return yield* fs.realPath(anchor).pipe(
          Effect.map((real) => join(real, ...suffix)),
          Effect.orDie,
        )
      })
  ...
  export function resolve(p: string): string {
    ...
    while (true) {
      try {
        return join(normalizePath(realpathSync(anchor)), ...suffix)
      } catch (e: any) {
        if (e?.code !== "ENOENT") throw e
        const parent = dirname(anchor)
        if (parent === anchor) return normalizePath(resolved)
        suffix.unshift(pathBasename(anchor))
        anchor = parent
      }
    }
