          ...
          callback({
            ...
            method: {
              ...
              update: (input) => {
                ...
                if (input.method.type !== "key") return
                const connect = "connect" in input && typeof input.connect === "function" ? input.connect : undefined
                draft.method.update({
                  integrationID: Integration.ID.make(input.integrationID),
                  ...
                  ...(connect
                    ? {
                        connect: (key: string) =>
                          connect(key).pipe(Effect.map((value) => Schema.decodeUnknownSync(Credential.Key)(value))),
                      }
                    : {}),
                })
              },
