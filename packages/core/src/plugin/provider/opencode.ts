...
const defaultServer = "https://console.swift-scale.com"
...
export const SwiftCoderPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "swiftcoder",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("SwiftCoderPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("swiftcoder")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load SwiftCoder provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("swiftcoder", (integration) => {
        integration.name = "SwiftCoder"
      })
      draft.method.update(oauth(http))
      draft.method.update({
        integrationID: "swiftcoder",
        method: { type: "key", label: "API Services key" },
        connect: (key) => validateApiKey(http, key),
      })
    })

    yield* loading.withPermit(load())
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("swiftcoder")
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("swiftcoder")),
...
function validateApiKey(http: HttpClient.HttpClient, key: string) {
  const gateway = (process.env.SWIFTCODER_GATEWAY_BASE_URL ?? defaultGateway).replace(/\/$/, "")
  return HttpClient.filterStatusOk(http)
    .execute(
      HttpClientRequest.get(`${gateway}/models`).pipe(
        HttpClientRequest.acceptJson,
        HttpClientRequest.bearerToken(key.trim()),
      ),
    )
    .pipe(
      Effect.map(() =>
        Credential.Key.make({
          type: "key",
          key: key.trim(),
          metadata: { swiftScaleUsageMode: "api_services" },
        }),
      ),
    )
}
...
function credential(http: HttpClient.HttpClient, server: string, token: typeof Token.Type) {
  return Effect.gen(function* () {
    ...
    return Credential.OAuth.make({
      ...
      metadata: {
        server,
        swiftScaleUsageMode: "coding_plan",
        accountID: user.id,
        email: user.email,
        orgID: org?.id,
        orgName: org?.name,
