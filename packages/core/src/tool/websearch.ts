...
export class ConfigService extends Context.Service<ConfigService, Config>()("@swiftcoder/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.SWIFTCODER_WEBSEARCH_PROVIDER === "exa" || process.env.SWIFTCODER_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.SWIFTCODER_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("SWIFTCODER_EXPERIMENTAL") || truthy("SWIFTCODER_ENABLE_EXA") || truthy("SWIFTCODER_EXPERIMENTAL_EXA"),
    enableParallel: truthy("SWIFTCODER_ENABLE_PARALLEL") || truthy("SWIFTCODER_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `swiftcoder/${InstallationVersion}`,
