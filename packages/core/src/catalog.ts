...
export class Service extends Context.Service<Service, Interface>()("@swiftcoder/v2/Catalog") {}
  ...
  Effect.gen(function* () {
    ...
    const state = State.create<Data, Draft>({
      ...
      finalize: Effect.fn("CatalogV2.finalize")(function* (catalog) {
        ...
        const defaultModel = swiftCoder
          ? globalThis.Array.from(swiftCoder.models.values())
              .filter((model) => model.enabled && model.status === "active")
              .sort((a, b) => b.time.released - a.time.released)[0]
          : undefined
        if (defaultModel) catalog.model.default.set(ProviderV2.ID.swiftcoder, defaultModel.id)
    ...
    const result: Interface = {
      ...
      model: {
        ...
        small: Effect.fn("CatalogV2.model.small")(function* (providerID) {
          ...
          if (providerID === ProviderV2.ID.swiftcoder) {
