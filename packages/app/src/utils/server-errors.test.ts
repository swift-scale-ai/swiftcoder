...
import type { SessionNotFoundError } from "@swiftscale/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em swiftcoder.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
