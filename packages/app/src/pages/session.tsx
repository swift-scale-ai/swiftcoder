import type { FilePart, Project, UserMessage, VcsFileDiff } from "@swiftscale/sdk/v2"
...
import type { SessionReviewLineComment } from "@swiftscale/session-ui/session-review"
import { ResizeHandle } from "@swiftscale/ui/resize-handle"
import { Select } from "@swiftscale/ui/select"
import { SelectV2 } from "@swiftscale/ui/v2/select-v2"
import { isScrollKeyTarget, scrollKey, scrollKeyOwner } from "@swiftscale/ui/scroll-view"
import { Tabs } from "@swiftscale/ui/tabs"
import { ButtonV2 } from "@swiftscale/ui/v2/button-v2"
import { createAutoScroll } from "@swiftscale/ui/hooks"
import { previewSelectedLines } from "@swiftscale/session-ui/pierre/selection-bridge"
import { Button } from "@swiftscale/ui/button"
import { showToast } from "@/utils/toast"
import { base64Encode, checksum } from "@swiftscale/core/util/encode"
...
import { useSessionPlacement } from "@/context/session-placement"
...
import { SessionReviewEmptyChangesV2 } from "@swiftscale/session-ui/v2/session-review-empty-changes-v2"
import { SessionReviewEmptyNoGitV2 } from "@swiftscale/session-ui/v2/session-review-empty-no-git-v2"
import { SessionReviewV2SidebarToggle } from "@swiftscale/session-ui/v2/session-review-v2"
...
export default function Page() {
  ...
  const serverSDK = useServerSDK()
  const settings = useSettings()
  const sessionPlacement = useSessionPlacement()
  const platform = usePlatform()
  const prompt = usePrompt()
  const comments = useComments()
  const command = useCommand()
  const terminal = useTerminal()
  const [searchParams, setSearchParams] = useSearchParams<{ prompt?: string; view?: string }>()
            ...
            <Show when={messagesReady() ? params.id : undefined} keyed>
              {(_id) => (
                <MessageTimeline
                  ...
                  centered={centered()}
                  sectionTitle={
                    searchParams.view === "chat" || (params.id !== undefined && sessionPlacement.get(params.id) === "chat")
                      ? language.t("sidebar.chat")
                      : undefined
                  }
                  setContentRef={(el) => {
                    content = el
