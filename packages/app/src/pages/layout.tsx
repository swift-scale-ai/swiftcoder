...
import { ResizeHandle } from "@swiftscale/ui/resize-handle"
...
import { Session } from "@swiftscale/sdk/v2/client"
...
import { Binary } from "@swiftscale/core/util/binary"
import { retry } from "@swiftscale/core/util/retry"
...
import { DialogSwiftScaleAccount } from "@/components/dialog-swiftscale-account"
import { SessionPlacementProvider } from "@/context/session-placement"

import { useDialog } from "@swiftscale/ui/context/dialog"
import { useTheme, type ColorScheme } from "@swiftscale/ui/theme/context"
...
export default function LegacyLayout(props: ParentProps) {
  ...
  const [store, setStore, , ready] = persisted(
    ...
    createStore({
      ...
      sessionPlacement: {} as Record<string, SidebarSessionPlacement>,
      sessionOrdinal: {} as Record<string, number>,
      sessionScopeSequence: {} as Record<string, number>,
  ...
  let desktopSidebarInitialized = false
  createEffect(() => {
    if (!layoutReady() || desktopSidebarInitialized) return
    desktopSidebarInitialized = true
    layout.sidebar.open()
  })
  const platform = usePlatform()
  const sidebarOpened = () => platform.platform === "desktop" || layout.sidebar.opened()
  ...
  const currentDir = createMemo(() => {
    ...
    if (active.type === "draft" && active.directory) return active.directory
  ...
  const [autoselecting] = createResource(async () => {
    ...
    // The tab-based layout restores its own active draft/session. Running the
    // legacy project autoselection here creates a fresh draft on every launch.
  ...
  createEffect(() => {
    for (const session of chatSessions()) prefetchSession(session, "high")
  })
  ...
  async function archiveSession(session: Session) {
    ...
    const [directoryStore, setDirectoryStore] = serverSync().child(session.directory)
    const sessions = directoryStore.session ?? []
  ...
  async function createProjectSession(directory: string, placement: SidebarSessionPlacement = "project") {
    const key = pathKey(directory)
    if (creatingSessions.has(key)) return
    creatingSessions.add(key)

    try {
      const sequenceScope = placement === "chat" ? "chat" : `project:${key}`
      const ordinal = (store.sessionScopeSequence[sequenceScope] ?? 0) + 1
      setStore("sessionScopeSequence", sequenceScope, ordinal)
      const session = await serverSDK().api.session.create({ location: { directory } }).then(normalizeSessionInfo)
      setStore("sessionPlacement", session.id, placement)
      setStore("sessionOrdinal", session.id, ordinal)
      serverSync().session.remember(session)
      const [, setDirectoryStore] = serverSync().child(directory, { bootstrap: true })
      setDirectoryStore("session", (list: Session[]) => {
        const result = Binary.search(list, session.id, (item) => item.id)
        if (result.found) return list.map((item) => (item.id === session.id ? session : item))
        const next = [...list]
      ...
      navigateWithSidebarReset(
        `/${base64Encode(directory)}/session/${session.id}${placement === "chat" ? "?view=chat" : ""}`,
      )
    } catch (cause) {
      showToast({
  ...
  function openAccount() {
    ...
    void import("@/components/dialog-settings").then((module) => {
      if (dialogDead || dialogRun !== run) return
      dialog.show(() => <module.DialogSettings defaultValue="account" />)
    })
  }
  ...
  const handleDeepLinks = (urls: string[]) => {
    ...
    const billing = collectBillingCallbackDeepLinks(urls).at(0)
    if (billing?.result === "cancel") {
      showToast({ title: "Plan change canceled", description: "Your SwiftScale plan was not changed." })
    }
    if (billing?.result === "success") {
      showToast({
        title: "Checking your SwiftScale plan",
        description: "SwiftCoder will confirm the result with SwiftScale before enabling new capacity.",
      })
      dialog.show(() => <DialogSwiftScaleAccount refreshEntitlements />)
    }
  ...
  createEffect(() => {
    document.documentElement.style.setProperty(
      "--dialog-left-margin",
      `${layout.sidebar.opened() ? Math.min(Math.max(layout.sidebar.width(), 200), 320) : 48}px`,
    )
  })

  const side = createMemo(() => {
    const width = layout.sidebar.width()
    return Number.isFinite(width) ? Math.min(Math.max(width, 200), 320) : 212
  })
  ...
  const projectSidebarCtx: ProjectSidebarContext = {
    ...
    renameProject: (project) => {
      ...
      requestAnimationFrame(() =>
        openEditor(`project:${project.id ?? pathKey(project.worktree)}`, displayName(project)),
      )
    },
    ...
    workspaceIds,
    workspaceLabel,
    includeSession: (session) => belongsToProject(store.sessionPlacement[session.id]),
    sessionDisplayTitle: (session) => numberedDefaultSessionTitle(session.title, store.sessionOrdinal[session.id]),
    sessionProps: {
      navList: currentSessions,
      sidebarExpanded,
      clearHoverProjectSoon,
  ...
  const SidebarPanel = (panelProps: {
    ...
    const projectId = createMemo(() => {
      const item = project()
      if (!item) return ""
      return item.id ?? pathKey(item.worktree)
    })
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      recents={() =>
        chatSessions().map((session) => (
          <SessionItem
            ...
            showMeta
            displayTitle={() =>
              chatPreview(session) || numberedDefaultSessionTitle(session.title, store.sessionOrdinal[session.id])
            }
      ...
      newChatLabel={language.t("command.session.new")}
      newChatKeybind={() => command.keybind("session.new")}
      onNewTask={() => {
        const dir = currentDir() || currentProject()?.worktree || layout.projects.list()[0]?.worktree
        if (dir) {
          void createProjectSession(dir)
          return
        }
      ...
      onNewChat={() => {
        const dir = currentDir() || currentProject()?.worktree || layout.projects.list()[0]?.worktree
        if (dir) {
          void createProjectSession(dir, "chat")
          return
        }
        chooseProject()
      }}
      ...
      onOpenHelp={() => platform.openExternal("https://swift-scale.com/contact/?product=swiftcoder")}
  ...
  return (
    <div
      data-layout="swiftcoder-workbench"
        ...
        <div class="flex-1 min-h-0 relative">
          <div class="size-full relative overflow-x-hidden">
            <nav
              ...
              classList={{
                block: platform.platform === "desktop",
                "hidden sm:block": platform.platform !== "desktop",
            ...
            <Show when={sidebarOpened()}>
              <div
                class={`${platform.platform === "desktop" ? "block" : "hidden sm:block"} absolute inset-y-0 z-30 w-0 overflow-visible`}
            ...
            <div
              class={`${platform.platform === "desktop" ? "block" : "hidden sm:block"} pointer-events-none absolute top-0 end-0 z-0 border-t border-border-weaker-base`}
              style={{ "inset-inline-start": "calc(4rem + 12px)" }}
            />

            <div class={platform.platform === "desktop" ? "hidden" : "sm:hidden"}>
              <div
                classList={{
            ...
            <div
              classList={{
                "absolute inset-y-0 end-0 start-[var(--main-left)]": platform.platform === "desktop",
                "absolute inset-0 sm:inset-y-0 sm:end-0 sm:start-[var(--main-left)]": platform.platform !== "desktop",
                "z-20": true,
              ...
              style={{
                "--main-left": sidebarOpened() ? `${side()}px` : "4rem",
              }}
            >
              <main
                classList={{
                  ...
                  "border-s": platform.platform === "desktop",
                  "sm:border-s": platform.platform !== "desktop",
                }}
              >
                <Show when={!autoselecting.loading} fallback={<div class="size-full" />}>
                  <SessionPlacementProvider get={(sessionID) => store.sessionPlacement[sessionID]}>
                    {props.children}
                  </SessionPlacementProvider>
                </Show>
              </main>
            </div>
            ...
            <div
              classList={{
                "flex absolute inset-y-0 start-16 z-30": platform.platform === "desktop",
                "hidden sm:flex absolute inset-y-0 start-16 z-30": platform.platform !== "desktop",
            ...
            <div
              classList={{
                "block pointer-events-none absolute inset-y-0 end-0 z-25 overflow-hidden":
                  platform.platform === "desktop",
                "hidden sm:block pointer-events-none absolute inset-y-0 end-0 z-25 overflow-hidden":
                  platform.platform !== "desktop",
