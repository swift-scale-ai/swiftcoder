...
import { Card } from "@swiftscale/ui/card"
import {
  ContextToolGroup,
  Message,
  MessageDivider,
  ...
  partDefaultOpen,
  type UserActions,
} from "@swiftscale/session-ui/message-part"
import { DiffChanges } from "@swiftscale/ui/diff-changes"
import { FileIcon } from "@swiftscale/ui/file-icon"
...
import { IconButtonV2 } from "@swiftscale/ui/v2/icon-button-v2"
...
import { DialogFooter, DialogHeader, DialogTitleGroup, DialogV2 } from "@swiftscale/ui/v2/dialog-v2"
import { InlineInput } from "@swiftscale/ui/inline-input"
import { ButtonV2 } from "@swiftscale/ui/v2/button-v2"
import { SessionRetry } from "@swiftscale/session-ui/session-retry"
import { isScrollKeyTarget, scrollKey, scrollKeyOwner, ScrollView } from "@swiftscale/ui/scroll-view"
import { StickyAccordionHeader } from "@swiftscale/ui/sticky-accordion-header"
import { TextField } from "@swiftscale/ui/text-field"
import { TextReveal } from "@swiftscale/ui/text-reveal"
import { TextShimmer } from "@swiftscale/ui/text-shimmer"
import type {
  AssistantMessage,
  ...
  ToolPart,
  UserMessage,
} from "@swiftscale/sdk/v2"
import { showToast } from "@/utils/toast"
import { downloadSessionExport, fetchSessionExport, sessionExportFilename } from "@/utils/session-export"
import { getDirectory, getFilename } from "@swiftscale/core/util/path"
import { Popover as KobaltePopover } from "@kobalte/core/popover"
import { normalize } from "@swiftscale/session-ui/session-diff"
import { useFileComponent } from "@swiftscale/ui/context/file"
...
export function MessageTimeline(props: {
  ...
  const [swiftScaleAuthStatus, setSwiftScaleAuthStatus] = createSignal<SwiftScaleAuthStatus>()

  onMount(() => {
    const auth = platform.swiftScaleAuth
    if (!auth) return
    void auth.status().then(setSwiftScaleAuthStatus)
    const unsubscribe = auth.subscribe(setSwiftScaleAuthStatus)
    onCleanup(unsubscribe)
  })

  const [listRoot, setListRoot] = createSignal<HTMLDivElement>()
  const [continuingInterrupted, setContinuingInterrupted] = createSignal(false)
  ...
  const projectTitle = createMemo(
    () => props.sectionTitle || sync().project?.name || getFilename(sdk().directory) || "Default Project",
  )
  ...
  const continueInterrupted = async (userMessageID: string) => {
    const id = sessionID()
    const message = messageByID().get(userMessageID)
    if (!id || message?.role !== "user" || continuingInterrupted()) return
    setContinuingInterrupted(true)
    try {
      await sdk().api.session.prompt({
        sessionID: id,
        agent: message.agent,
        model: message.model,
        text: "Continue from the preserved partial result. Re-check the repository state before running any command or mutation; do not assume interrupted tools completed.",
      })
    } catch (error) {
      showToast({
      ...
      setContinuingInterrupted(false)
    }
  }

  const handleSwiftScaleAuthError = (userMessageID: string, message: string) => {
    const reauthenticate = isSwiftScaleReauthenticationError(message)
    const presentation = swiftScaleAuthErrorPresentation(swiftScaleAuthStatus(), { reauthenticate })
    if (presentation.kind === "checking") return
    if (reauthenticate) {
      void platform.swiftScaleAuth?.login().catch((error) => {
        showToast({
          title: "SwiftScale sign-in failed",
          description: error instanceof Error ? error.message : String(error),
        })
      })
      return
    ...
    if (presentation.kind === "retry") {
      void continueInterrupted(userMessageID)
      return
    }
  ...
  const renderTimelineRow = (row: Accessor<TimelineRow.TimelineRow>, onSizeChange?: () => void) => {
    switch (row()._tag) {
          ...
          <TimelineRowFrame row={turnDividerRow}>
            <div data-slot="session-turn-message-container" class="w-full px-4 md:px-5">
              <div data-slot="session-turn-compaction">
                ...
                <Show
                  when={
                    turnDividerRow().label === "interrupted" &&
                    activeMessageID() === turnDividerRow().userMessageID &&
                ...
                >
                  <div class="flex justify-center pt-2">
                    <Button
                      size="small"
                      variant="secondary"
                      disabled={continuingInterrupted()}
                      onClick={() => void continueInterrupted(turnDividerRow().userMessageID)}
                    >
      ...
      case "Error": {
        ...
        const authPresentation = createMemo(() =>
          swiftScaleAuthErrorPresentation(swiftScaleAuthStatus(), {
            reauthenticate: isSwiftScaleReauthenticationError(errorRow().text),
          }),
        )
        return (
          <TimelineRowFrame row={errorRow}>
            <div data-slot="session-turn-message-container" class="w-full px-4 md:px-5">
              <Show
                when={isSwiftScaleAuthError(errorRow().text)}
                fallback={
                  <Show
                    when={isSwiftScaleAvailabilityError(errorRow().text)}
                    fallback={
                  ...
                  >
                    <div
                      data-component="swiftscale-service-unavailable"
                      class="flex w-full items-center gap-3 rounded-md border border-border-weak-base bg-surface-raised-base px-4 py-3"
                    >
                      <div class="flex size-8 shrink-0 items-center justify-center rounded-md bg-surface-warning-weak text-icon-warning-base">
                        <Icon name="warning" size="small" />
                      </div>
                      <div class="min-w-0 flex-1">
                        <div class="text-13-medium text-text-strong">SwiftScale service temporarily unavailable</div>
                        <div class="mt-1 text-12-regular text-text-weak">
                          The service may be restarting. Wait briefly, then retry this request.
                        </div>
                      </div>
                      <ButtonV2
                        variant="outline"
                        class="shrink-0"
                        disabled={continuingInterrupted()}
                        onClick={() => void continueInterrupted(errorRow().userMessageID)}
                      >
                        Retry
                      </ButtonV2>
                    </div>
              ...
              >
                <div
                  data-component="swiftscale-auth-required"
                  class="flex w-full items-center gap-3 rounded-md border border-border-weak-base bg-surface-raised-base px-4 py-3"
                >
                  <div class="flex size-8 shrink-0 items-center justify-center rounded-md bg-surface-critical-weak text-icon-critical-base">
                    <Icon name="warning" size="small" />
                  </div>
                  <div class="min-w-0 flex-1">
                    <div class="text-13-medium text-text-strong">{authPresentation().title}</div>
                    <div class="mt-1 text-12-regular text-text-weak">{authPresentation().description}</div>
                  </div>
                  <Show when={authPresentation().action}>
                    {(action) => (
                      <ButtonV2
                        variant="outline"
                        class="shrink-0"
                        disabled={authPresentation().kind === "retry" && continuingInterrupted()}
                        onClick={() => handleSwiftScaleAuthError(errorRow().userMessageID, errorRow().text)}
                      >
                        {action()}
                      </ButtonV2>
                    )}
                ...
                <div class="flex items-center min-w-0 flex-1 w-full">
                  <Show when={!parentID()}>
                    <span
                      ...
                      class="truncate text-[12px] font-semibold text-v2-text-text-base"
                    >
                      {projectTitle()}
                    </span>
                    <span
                      data-slot="session-title-separator"
                      class="px-1 text-[11px] text-v2-text-text-faint"
                      aria-hidden="true"
                    >
                      /
                    </span>
