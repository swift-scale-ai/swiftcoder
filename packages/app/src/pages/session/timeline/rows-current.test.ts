...
mock.module("@swiftscale/session-ui/message-part", () => ({
  renderable: () => true,
...
describe("current session timeline rows", () => {
  ...
  test("projects a 2,000-message session within the Phase 3 renderer budget", () => {
    const source = Array.from({ length: 1_000 }, (_, index) => {
      const turn = index * 2
      return [
        { id: `msg_${turn}`, type: "user", text: `question ${index}`, time: { created: turn } },
        {
          id: `msg_${turn + 1}`,
          type: "assistant",
          agent: "build",
          model: { id: "swiftscale", providerID: "swiftscale" },
          content: [{ type: "text", text: `answer ${index}` }],
          time: { created: turn + 1, completed: turn + 2 },
        },
      ]
    }).flat() as SessionMessageInfo[]
    const normalized = normalizeSessionMessages("ses_long", source)
    const messages = new Map(normalized.messages.map((message) => [message.id, message]))
    ...
    expect(result.rows).toHaveLength(2_999)
    expect(durationMs).toBeLessThanOrEqual(500)
    console.log(`Phase 3 long-session projection: ${durationMs.toFixed(1)} ms / 500 ms`)
  })
})
