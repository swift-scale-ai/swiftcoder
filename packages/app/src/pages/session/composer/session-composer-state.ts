...
import type { PermissionRequest, QuestionRequest, Todo } from "@swiftscale/sdk/v2"
...
export function createSessionComposerController(options?: { closeMs?: number | (() => number) }) {
  ...
  const decide = (response: "once" | "always" | "reject") => {
    ...
    sdk()
      ...
      .then(() => {
        if (response !== "always" || !directory) return
        permission.enableAutoAccept(perm.sessionID, directory)
      })
      .catch((err: unknown) => {
