...
import { List } from "@swiftscale/ui/list"
import { Tooltip } from "@swiftscale/ui/tooltip"
import { Icon } from "@swiftscale/ui/v2/icon"
import { Tag as TagV2 } from "@swiftscale/ui/v2/badge-v2"
...
import { effectiveSwiftScaleProductMode, filterSwiftScaleModelsByProductMode } from "./swiftscale-model-access"
...
const SWIFTSCALE_PROVIDER_ID = "swiftcoder"
...
const ModelList: Component<{
  ...
  const models = createMemo(() => {
    const available = model
      ...
      .filter((m) => m.provider.id === SWIFTSCALE_PROVIDER_ID)
      .filter((m) => (props.provider ? m.provider.id === props.provider : true))
    if (modelEntitlements.loading()) return []
    const products = modelEntitlements.products()
    const mode = effectiveSwiftScaleProductMode(products, local.productMode.current())
    return filterSwiftScaleModelsByProductMode(available, products, mode)
  })
...
function createModelSelectorController(input: {
  ...
  const allModels = createMemo(() => {
    const available = model
      ...
      .filter((item) => item.provider.id === SWIFTSCALE_PROVIDER_ID)
...
export const DialogSelectModel: Component<{ provider?: string; model?: ModelState }> = (props) => {
  ...
  return (
    <Dialog title={language.t("dialog.model.select.title")}>
      <ModelList provider={SWIFTSCALE_PROVIDER_ID} model={props.model} onSelect={() => dialog.close()} />
    </Dialog>
  )
}
