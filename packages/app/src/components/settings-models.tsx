...
import { Switch } from "@swiftscale/ui/switch"
            ...
            <For each={list.grouped.latest}>
              {(group) => (
                <div class="flex flex-col gap-1">
                  <Show when={group.category !== "swiftcoder"}>
                    ...
                    {(familyGroup) => (
                      <div class="flex flex-col gap-1.5">
                        <Show when={familyGroup.family}>
                          {(family) => (
                            <div class="flex items-center gap-2 pb-1 pt-2">
                              <ProviderIcon
                                id={swiftScaleModelFamilyProviderID(family())}
                                class="size-5 shrink-0 icon-strong-base"
                              />
                              <span class="text-14-medium text-text-strong">{family()}</span>
                            </div>
                          )}
                        </Show>
                        <SettingsList>
