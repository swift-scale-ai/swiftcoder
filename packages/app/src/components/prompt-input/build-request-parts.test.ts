...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.swiftcoderComment &&
          (part.metadata.swiftcoderComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("preserves an external attachment source path for the model", () => {
    const result = buildRequestParts({
      ...
      images: [
        {
          type: "image",
          id: "img_external",
          filename: "swiftcoder.global.dat",
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.swiftscale.swiftcoder.beta\\swiftcoder.global.dat",
          mime: "text/plain",
      ...
      sessionDirectory: "C:\\Repos\\sst\\swiftcoder",
    })

    expect(result.requestParts.find((part) => part.type === "file")?.filename).toBe(
      "C:\\Users\\Luke\\AppData\\Roaming\\ai.swiftscale.swiftcoder.beta\\swiftcoder.global.dat",
    )
  })
