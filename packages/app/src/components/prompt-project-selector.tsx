...
import { ProjectAvatar } from "@swiftscale/ui/v2/project-avatar-v2"
...
function ProjectTrigger(props: ComponentProps<"button"> & { controller: PromptProjectController; iconOnly?: boolean }) {
  const [local, rest] = splitProps(props, ["controller", "iconOnly", "class", "classList", "onClick", "onKeyDown"])
  ...
  return (
    <button
      ...
      classList={{
        ...local.classList,
        "flex items-center justify-center rounded-[6px] border border-border-weak-base bg-background-stronger text-icon-strong transition-colors hover:bg-surface-base-hover focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-border-strong-base":
          local.iconOnly,
        "size-10": local.iconOnly,
        "flex h-7 min-w-0 max-w-[203px] items-center gap-1.5 rounded-sm px-1.5 transition-colors focus-visible:bg-v2-overlay-simple-overlay-hover focus-visible:outline-none":
          !local.iconOnly,
        "hover:bg-v2-overlay-simple-overlay-hover": !local.iconOnly && !local.controller.open(),
        "bg-v2-overlay-simple-overlay-pressed": !local.iconOnly && local.controller.open(),
        "text-v2-text-text-muted": !local.iconOnly && local.controller.open(),
      }}
      aria-label={local.iconOnly ? local.controller.labels.search() : rest["aria-label"]}
      title={local.iconOnly ? local.controller.labels.search() : rest.title}
