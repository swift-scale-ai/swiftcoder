...
import { SelectV2 } from "@swiftscale/ui/v2/select-v2"
import { Switch } from "@swiftscale/ui/v2/switch-v2"
import { TextInputV2 } from "@swiftscale/ui/v2/text-input-v2"
        ...
        <SettingsRowV2
          ...
          description={
            <>
              ...
              <ExternalLink class="settings-v2-link" href="https://swift-scale.com/docs/themes/">
  ...
  return (
    ...
    >
      <SelectV2
        ...
        listboxStyle={{ "max-height": "min(22rem, calc(100vh - 8rem))" }}
        options={options()}
        placement="bottom-end"
        gutter={6}
...
export const SettingsGeneralV2: Component<{
  ...
  const GeneralSection = () => (
    <div class="settings-v2-section">
      <SettingsListV2>
        ...
        <Show when={mobile() && import.meta.env.VITE_SWIFTCODER_CHANNEL !== "prod"}>
          <SettingsRowV2
  ...
  const SupportSection = () => {
    const exportDiagnostics = async () => {
      try {
        const path = await platform.exportDebugLogs?.()
        if (!path) return
        showToast({
          title: language.t("settings.support.diagnostics.exported.title"),
          description: language.t("settings.support.diagnostics.exported.description", { path }),
          variant: "success",
        })
      } catch (error) {
        showToast({
    ...
    return (
      <div class="settings-v2-section">
        <h3 class="settings-v2-section-title">{language.t("settings.general.section.supportPrivacy")}</h3>
        <SettingsListV2>
          <SettingsRowV2
            title={language.t("settings.support.privacy.title")}
            description={language.t("settings.support.privacy.description")}
          >
            <ButtonV2
              size="normal"
              variant="neutral"
              onClick={() => platform.openExternal("https://swift-scale.com/privacy")}
            >
              {language.t("settings.support.action.open")}
            </ButtonV2>
          </SettingsRowV2>
          <SettingsRowV2
            title="Share product analytics"
            description="Send fixed, content-free usage events. Source code, prompts, paths, and command output are never included."
          >
            <Switch
              checked={settings.general.productAnalytics()}
              onChange={(checked) => settings.general.setProductAnalytics(checked)}
            />
          </SettingsRowV2>
          <SettingsRowV2
            title={language.t("settings.support.licenses.title")}
            description={language.t("settings.support.licenses.description")}
          >
            <ButtonV2
              size="normal"
              variant="neutral"
              onClick={() => platform.openExternal("https://swift-scale.com/swiftcoder/legal/open-source")}
            >
              {language.t("settings.support.action.open")}
            </ButtonV2>
          </SettingsRowV2>
          <SettingsRowV2
            title={language.t("settings.support.diagnostics.title")}
            description={language.t("settings.support.diagnostics.description")}
          >
            <ButtonV2 size="normal" variant="neutral" onClick={() => void exportDiagnostics()}>
              {language.t("settings.support.diagnostics.action")}
            </ButtonV2>
          </SettingsRowV2>
          <SettingsRowV2
            title={language.t("settings.support.deleteData.title")}
            description={language.t("settings.support.deleteData.description")}
          >
            <ButtonV2 size="normal" variant="danger" onClick={() => void platform.deleteLocalData?.()}>
              {language.t("settings.support.deleteData.action")}
            </ButtonV2>
          </SettingsRowV2>
        </SettingsListV2>
      </div>
