import { useFilteredList } from "@swiftscale/ui/hooks"
...
import { isSwiftCoderTextModel } from "../swiftscale-model-access"
...
export const SettingsModelsV2: Component = () => {
  ...
  const list = useFilteredList<ModelItem>({
    items: (_filter) =>
      models.list().filter((model) => model.provider.id !== "swiftcoder" || isSwiftCoderTextModel(model)),
          ...
          >
            <For each={list.grouped.latest}>
              {(group) => {
                ...
                const familyGrouped = () => group.category === "swiftcoder"
                const expanded = () => familyGrouped() || searching() || !store.collapsed[group.category]
                ...
                return (
                  ...
                  >
                    ...
                    <Show when={expanded()}>
                      <For each={modelGroups(group.items)}>
                        {(familyGroup) => (
                          <div class="flex flex-col gap-1.5">
                            <Show when={familyGroup.family}>
                              {(family) => (
                                <div class="flex items-center gap-2 px-4 pb-1 pt-3">
                                  <ProviderIcon
                                    id={swiftScaleModelFamilyProviderID(family())}
                                    width={PROVIDER_ICON_SIZE}
                                    height={PROVIDER_ICON_SIZE}
                                    class="shrink-0"
                                  />
                                  <span class="settings-v2-section-title">{family()}</span>
                                </div>
                              )}
                            </Show>
                            <SettingsListV2>
                              <For each={familyGroup.models}>
                                {(item) => {
