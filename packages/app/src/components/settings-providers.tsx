...
const SettingsProvidersContent: Component<{ onBack?: () => void }> = (props) => {
  ...
  const swiftScaleAuth = useSwiftScaleAuthStatus()
  const accountConnected = (item: ProviderItem) =>
    item.id === "swiftcoder" && swiftScaleAuth.status()?.state === "signed_in"
  ...
  const type = (item: ProviderItem) => {
    if (accountConnected(item)) return language.t("settings.providers.tag.account")
  ...
  const canDisconnect = (item: ProviderItem) =>
    !(item.id === "swiftcoder" && swiftScaleAuth.available && swiftScaleAuth.status() === undefined) &&
    !accountConnected(item) &&
    source(item) !== "env" &&
                ...
                {(item) => (
                  <div class="group flex flex-wrap items-center justify-between gap-4 min-h-16 py-3 border-b border-border-weak-base last:border-none">
                    ...
                    <Show
                      ...
                      fallback={
                        <span
                          class="text-14-regular text-text-base transition-opacity duration-200 pr-3 cursor-default"
                          classList={{
                            "opacity-100": accountConnected(item),
                            "opacity-0 group-hover:opacity-100": !accountConnected(item),
                          }}
                        >
                          {accountConnected(item)
