...
import { ProviderIcon } from "@swiftscale/ui/provider-icon"
...
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@swiftscale/ui/v2/dialog-v2"
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "swiftcoder"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.swiftcoder.tagline")}</div>
          </Show>
          <Show when={i.id === "swiftcoder"}>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["swiftcoder"]
  const all = createMemo(() => {
    language.locale()
    const query = store.filter.trim().toLowerCase()
    const values = [...providers.all().values()]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "swiftcoder"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t("dialog.provider.swiftcoder.tagline")}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "swiftcoder"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.swiftcoderZen.line1")}</div>
              <div>{language.t("provider.connect.swiftcoderZen.line2")}</div>
              <div>
                {language.t("provider.connect.swiftcoderZen.visit.prefix")}
                <ExternalLink
                  href="https://swift-scale.com/account/api-keys"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.swiftcoderZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.swiftcoderZen.visit.suffix")}
              </div>
            </div>
          </Show>
          <form onSubmit={handleSubmit} class="flex flex-col items-start gap-5 self-stretch">
            ...
            <ButtonV2
              type="submit"
              variant={formStore.busy ? "loading" : "contrast"}
              disabled={formStore.busy}
              data-action="provider-connect-submit"
            >
              {formStore.busy ? "Connecting..." : language.t("common.continue")}
            </ButtonV2>
          </form>
        </div>
      )
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "swiftcoder"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.swiftcoderZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.swiftcoderZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.swiftcoderZen.visit.prefix")}
                <ExternalLink href="https://swift-scale.com/account/api-keys" tabIndex={-1}>
                  {language.t("provider.connect.swiftcoderZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.swiftcoderZen.visit.suffix")}
              </div>
