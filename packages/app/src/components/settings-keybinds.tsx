...
const IconV2 = lazy(() => import("@swiftscale/ui/v2/icon").then((module) => ({ default: module.Icon })))
...
function listFor(command: Pick<CommandContext, "catalog" | "options">, palette: string) {
  const out = new Map<string, KeybindMeta>()
  out.set(PALETTE_ID, { title: palette, group: "General" })

  const include = (id: string) =>
    !/^project\.[1-9]$/.test(id) && !id.startsWith("theme.set.") && !id.startsWith("language.set.")

  for (const opt of command.catalog) {
    if (opt.id.startsWith("suggested.")) continue
    if (opt.hidden) continue
    if (!include(opt.id)) continue
...
export function createKeybindSettingsController(
  ...
  const list = createMemo(() => {
    language.locale()
    return listFor(input.command, language.t("command.palette"))
  })
    ...
    <>
      <div class="settings-v2-tab-header settings-v2-tab-header--stacked">
        <div class="settings-v2-tab-header-row">
          ...
          <ButtonV2 type="button" variant="ghost" onClick={props.onReset}>
...
export const SettingsKeybinds: Component<{ v2?: boolean }> = (props) => {
  ...
  const list = createMemo(() => {
    language.locale()
    return listFor(command, language.t("command.palette"))
  })
      ...
      <div class="sticky top-0 z-10 bg-[linear-gradient(to_bottom,var(--surface-stronger-non-alpha)_calc(100%_-_24px),transparent)]">
        <div class="flex flex-col gap-4 pt-6 pb-6 max-w-[720px]">
          <div class="flex items-center justify-between gap-4">
            ...
            <Button type="button" size="small" variant="secondary" onClick={resetAll}>
              {language.t("settings.shortcuts.reset.button")}
            </Button>
          </div>
