...
describe("approvalModeFor", () => {
  test("defaults new sessions to agent approval", () => {
    expect(approvalModeFor({}, {}, [session({ id: "new" })], "new", "/tmp/project")).toBe("agent")
  })

  test("inherits a parent session mode", () => {
    const directory = "/tmp/project"
    const sessions = [session({ id: "root" }), session({ id: "child", parentID: "root" })]
    const modes = { [`${base64Encode(directory)}/root`]: "agent" as const }

    expect(approvalModeFor(modes, {}, sessions, "child", directory)).toBe("agent")
  })

  test("falls back to a directory mode for a new session", () => {
    const directory = "/tmp/project"
    const modes = { [`${base64Encode(directory)}/*`]: "full" as const }

    expect(approvalModeFor(modes, {}, [session({ id: "new" })], "new", directory)).toBe("full")
  })
})
