...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: SwiftCoderClient, protocol?: Promise<ServerProtocol>) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("swiftcoder-test"))
