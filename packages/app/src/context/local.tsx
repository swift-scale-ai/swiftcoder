...
type Saved = {
  ...
  productMode: "coding" | "api_services"
}
...
const migrate = (value: unknown) => {
  ...
  const productMode = item.productMode === "api_services" ? "api_services" : "coding"

  if (item.session && typeof item.session === "object") return { session: item.session, productMode }
  if (!item.pick || typeof item.pick !== "object") return { session: {}, productMode }
...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const result = {
      ...
      model,
      productMode: {
        current: () => saved.productMode,
        set(value: "coding" | "api_services") {
          setSaved("productMode", value)
        },
      },
      agent,
      session: {
