...
export type SwiftScaleEntitlements = {
  ...
  usage: { level: "available" | "limited" | "exhausted"; resetsAt?: string }
  limits: { concurrentTasks: number; contextTier: "standard" | "extended" | "maximum" }
  service: { status: "operational" | "degraded" | "outage"; message?: string }
  products?: {
    coding: {
      enabled: boolean
      models: string[]
...
type PlatformBase = {
  ...
  /** SwiftScale desktop account authentication. Tokens never enter the renderer. */
  swiftScaleAuth?: {
  ...
  /** Delete all SwiftCoder application data after native confirmation (desktop only) */
  ...
  /** Record a content-free product health metric in desktop diagnostics. */
  recordProductMetric?(
    name: "activation.completed" | "task.completed" | "task.failed" | "billing.opened" | "support.opened",
  ): Promise<void>
