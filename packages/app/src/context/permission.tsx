...
import { approvalModeAutoRespond, type ApprovalMode } from "./permission-mode"
...
export const { use: usePermission, provider: PermissionProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    return {
      ...
      setApprovalMode(sessionID: string | undefined, directory: string, mode: ApprovalMode) {
        selected().setApprovalMode(sessionID, directory, mode)
      },
      permissionsEnabled,
...
function createServerPermissionState(input: { sdk: ServerSDK; sync: ServerSync }) {
  const [store, setStore, _, ready] = persisted(
    {
      ...
      migrate(value) {
        ...
        if (data.autoAccept && data.approvalMode) return value
          ...
          autoAccept:
            typeof data.autoAccept === "object" && data.autoAccept && !Array.isArray(data.autoAccept)
              ...
              : typeof data.autoAcceptEdits === "object" && data.autoAcceptEdits && !Array.isArray(data.autoAcceptEdits)
                ...
                : {},
          approvalMode:
            typeof data.approvalMode === "object" && data.approvalMode && !Array.isArray(data.approvalMode)
              ? data.approvalMode
              : {},
        }
      },
  ...
  function shouldAutoRespond(permission: PermissionRequest, directory?: string) {
    ...
    return approvalModeAutoRespond(mode, permission)
  }
  ...
  const api = {
    ...
    approvalMode(sessionID: string | undefined, directory: string) {
      if (meta.disposed) return "agent" as ApprovalMode
      if (!sessionID) {
        return store.approvalMode[directoryAcceptKey(directory)] ??
          (isDirectoryAutoAccepting(store.autoAccept, directory) ? "full" : "agent")
      }
      return approvalModeFor(store.approvalMode, store.autoAccept, sessions(directory), sessionID, directory)
    },
    setApprovalMode(sessionID: string | undefined, directory: string, mode: ApprovalMode) {
      if (meta.disposed) return
      ...
      list(directory)
        .then((permissions) => {
          ...
          for (const request of permissions) {
            ...
            void respondPending(request, directory)
          }
        })
        .catch(() => undefined)
    },
