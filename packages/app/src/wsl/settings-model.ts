...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-swiftcoder" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslSwiftCoderAction(check?: WslSwiftCoderCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installSwiftCoder"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateSwiftCoder"
}
...
export function addServerViewModel(input: {
  ...
  const swiftcoderCheck = selectedDistro ? (state?.swiftcoderChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.swiftcoderChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateSwiftCoder" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.swiftcoderMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installSwiftCoder" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  swiftcoderCheck: WslSwiftCoderCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingSwiftCoder =
    probingSelected ||
    (ready &&
      (!input.swiftcoderCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingSwiftCoder =
    input.state?.job?.kind === "install-swiftcoder" && input.state.job.distro === input.selectedDistro
  if (!ready || probingSwiftCoder) {
    return {
      variant: "contrast",
  ...
  if (!addServerSwiftCoderReady(input.swiftcoderCheck)) {
    const update = !!input.swiftcoderCheck?.resolvedPath && input.swiftcoderCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingSwiftCoder
        ? { key: "wsl.onboarding.updatingSwiftCoder" }
        : update
          ? { key: "wsl.onboarding.updateSwiftCoder" }
          : { key: "wsl.onboarding.installSwiftCoder" },
      disabled: !!input.state?.job || input.adding,
      action: "install-swiftcoder",
      loading: installingSwiftCoder,
...
function addServerSwiftCoderReady(check: WslSwiftCoderCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.swiftcoderChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.swiftcoderChecks[item.name]) return [`swiftcoder:${item.name}`]
    return []
  })
