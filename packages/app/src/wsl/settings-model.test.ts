...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when SwiftCoder needs attention", () => {
    expect(wslSwiftCoderAction(undefined)).toBeUndefined()
    expect(
      wslSwiftCoderAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installSwiftCoder")
    expect(
      wslSwiftCoderAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/swiftcoder",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateSwiftCoder")
    expect(
      wslSwiftCoderAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/swiftcoder",
  ...
  test("does not report ready when SwiftCoder is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        swiftcoderChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.swiftcoder/bin/swiftcoder",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "swiftcoder is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installSwiftCoder" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-swiftcoder")
  })
