...
import { currentSession, mockSwiftCoderServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/SwiftCoder/SubagentNavigation"
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "swiftcoder", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "swiftcoder.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("swiftcoder.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
