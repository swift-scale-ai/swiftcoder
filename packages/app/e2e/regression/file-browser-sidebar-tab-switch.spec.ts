...
const directory = "C:/SwiftCoder/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockSwiftCoderServer(page, {
    ...
    provider: {
      ...
      default: { providerID: "swiftcoder", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "swiftcoder.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "swiftcoder.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "swiftcoder.window.browser.dat:tabs",
