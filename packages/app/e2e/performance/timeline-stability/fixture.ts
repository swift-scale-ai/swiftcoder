...
import { Event } from "@swiftscale/schema/event"
import { SessionStatusEvent } from "@swiftscale/schema/session-status-event"
import { SessionV1 } from "@swiftscale/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/SwiftCoder/TimelineStability"
...
export const model = { providerID: "swiftcoder", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("swiftcoder.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["swiftcoder"],
    default: { providerID: "swiftcoder", modelID: "claude-opus-4-6" },
  }
}
