...
const RATE_LIMIT_PATTERN = /(?:\b429\b|rate[ _-]?limit|too many requests|请求(?:过于)?频繁)/i

export function retryMessageKey(message: string) {
  if (RATE_LIMIT_PATTERN.test(message)) return "ui.sessionTurn.retry.rateLimited" as const
  return undefined
}
...
export function SessionRetry(props: { status: SessionStatus; show?: boolean }) {
  ...
  const message = createMemo(() => {
    ...
    const key = retryMessageKey(current.message)
  ...
  const truncated = createMemo(() => {
    ...
    if (retryMessageKey(current.message)) return false
