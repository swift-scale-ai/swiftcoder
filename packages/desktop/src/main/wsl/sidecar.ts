...
import { type WslCommandLine, resolveWslSwiftCoder, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const swiftcoder = await resolveWslSwiftCoder(distro)
  if (!swiftcoder) throw new Error(nativeT("desktop.wsl.error.swiftcoderNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "swiftcoder"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export SWIFTCODER_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export SWIFTCODER_CLIENT=desktop",
    `export SWIFTCODER_SERVER_USERNAME=${shellEscape(username)}`,
    `export SWIFTCODER_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(swiftcoder)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
