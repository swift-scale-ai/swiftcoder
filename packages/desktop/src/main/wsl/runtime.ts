...
import type { WslDistroProbe, WslInstalledDistro, WslOnlineDistro, WslRuntimeCheck } from "@swiftscale/coder-app/wsl/types"
...
export async function installWslSwiftCoder(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https://swift-scale.com/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function resolveWslSwiftCoder(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.swiftcoder/bin/swiftcoder" ]; then printf "%s\\n" "$HOME/.swiftcoder/bin/swiftcoder"; fi',
        distro,
        opts,
      )
    ).stdout,
