...
export async function finishFirstLaunchOnboarding(createDefaultProject: boolean) {
  ...
  const testProject =
    process.env.SWIFTCODER_TEST_ONBOARDING === "1" && isAbsolute(process.env.SWIFTCODER_TEST_PROJECT_DIR ?? "")
      ? process.env.SWIFTCODER_TEST_PROJECT_DIR
      : undefined
  const defaultProject = createDefaultProject
    ? (testProject ?? join(app.getPath("documents"), DEFAULT_PROJECT_DIR))
    : null
