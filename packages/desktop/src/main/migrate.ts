...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.swiftscale.swiftcoder.dev",
  beta: "ai.swiftscale.swiftcoder.beta",
  prod: "ai.swiftscale.swiftcoder",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.swiftscale.swiftcoder.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `swiftcoder.settings.dat` is special: it maps to the `swiftcoder.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"swiftcoder.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // swiftcoder.settings.dat → the electron settings store ("swiftcoder.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "swiftcoder.global.dat").
  const storeName = filename === "swiftcoder.settings.dat" ? "swiftcoder.settings" : filename
