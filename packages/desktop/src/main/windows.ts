...
import { resolveThemeVariant } from "@swiftscale/ui/theme/resolve"
import type { DesktopTheme } from "@swiftscale/ui/theme/types"
import oc2ThemeJson from "@swiftscale/ui/theme/themes/oc-2.json"
...
import { initialWindowHeight, safeWindowURL, visibleWindowBounds } from "./window-state"
...
export function createMainWindow(id: string = randomUUID()) {
  ...
  const state = windowState({
    ...
    defaultWidth: 1280,
    defaultHeight: initialWindowHeight(primary.height),
  })
...
function windowDataFile(id: string) {
  return `swiftcoder.window.${id.replace(/[^a-zA-Z0-9._-]/g, "-")}.dat`
}
...
function addDocumentPolicy(response: Response, file: string) {
  ...
  headers.set(contentSecurityPolicyHeader, rendererContentSecurityPolicy)
...
function addRendererHeaders(value: string, headers: Record<string, any>) {
  if (isRendererUrl(value, true)) {
    ...
    if (!process.env.ELECTRON_RENDERER_URL) {
      upsertKeyValue(headers, contentSecurityPolicyHeader, [rendererContentSecurityPolicy])
    }
  }
}
