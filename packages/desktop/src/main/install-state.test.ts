...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier SwiftCoder launch", () => {
    expect(hasExistingAppState([file("swiftcoder.settings")])).toBe(true)
    expect(hasExistingAppState([file("swiftcoder.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("swiftcoder")])).toBe(true)
  })
})
