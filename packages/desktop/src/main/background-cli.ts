...
const desktopStateNames = [
  "com.swift-scale.swiftcoder.dev",
  "com.swift-scale.swiftcoder.beta",
  "com.swift-scale.swiftcoder",
  // Keep discovering background services created by builds using the legacy ID.
  "ai.swiftscale.swiftcoder.dev",
  "ai.swiftscale.swiftcoder.beta",
  "ai.swiftscale.swiftcoder",
]
...
export async function startBackgroundCli(logger: Logger, shellStateHome?: string, restart = false) {
  ...
  if (restart && found) {
    logger.log("restarting v2 CLI background service")
    await run(binary, ["service", "stop"], logger, { stateHome: daemonStateHome })
  }
  const started = await run(binary, ["service", "start"], logger, { stateHome: daemonStateHome })
  const url = parseLoopbackServerUrl(started)
  if (!url) throw new Error("SwiftCoder background service returned a non-loopback URL")
...
function serviceUrl(status: string) {
  if (URL.canParse(status)) return parseLoopbackServerUrl(status)
  if (!status.startsWith("running ")) return
  const url = status.slice("running ".length).trim()
  return parseLoopbackServerUrl(url)
}
...
function executableName() {
  return process.platform === "win32" ? "swiftcoder-cli.exe" : "swiftcoder-cli"
}
