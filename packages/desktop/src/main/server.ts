...
export async function spawnLocalServer(
  ...
  const child = utilityProcess.fork(sidecar, [], {
    cwd: process.cwd(),
    env: createSidecarEnv(options.authContent),
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`swiftcoder:${password}`).toString("base64")
...
function createSidecarEnv(authContent?: string): Record<string, string> {
  const env = Object.fromEntries(
    Object.entries(process.env).flatMap(([key, value]) => (value === undefined ? [] : [[key, String(value)]])),
  )
  delete env.DEBUG
  if (process.platform === "linux") delete env.LD_PRELOAD
  if (authContent) env.SWIFTCODER_AUTH_CONTENT = authContent
  return env
}
