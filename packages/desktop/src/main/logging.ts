...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `swiftcoder-debug-${stamp()}.zip`)
  try {
...
export function write(
  ...
  if (extra !== undefined) {
    scoped[level](redact(message), redactValue(extra))
    return
  }
  scoped[level](redact(message))
}
...
const redactValue = (value: unknown, key = ""): unknown => {
  if (/(authorization|cookie|token|api_?key|password|secret|prompt|input|content|answers?)/i.test(key)) {
    return "[REDACTED]"
  }
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "swiftcoder", "log"), join(app.getPath("userData"), "swiftcoder", "log")])]
}
