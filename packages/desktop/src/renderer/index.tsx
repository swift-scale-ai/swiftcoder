...
import { useTheme } from "@swiftscale/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.SWIFTCODER_CHANNEL === "prod" &&
...
const deepLinkEvent = "swiftcoder:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__SWIFTCODER__ ??= {}
  const pending = window.__SWIFTCODER__.deepLinks ?? []
  window.__SWIFTCODER__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `swiftcoder.desktop.window.${windowID}.last-active-url`
}
...
const createPlatform = (windowState: DesktopWindowState): Platform => {
  ...
  return {
    ...
    swiftScaleAuth: window.api.swiftScaleAuth,

    exportDebugLogs: () => window.api.exportDebugLogs(),
    deleteLocalData: () => window.api.deleteLocalData(),

    setForceFocus: (enabled) => window.api.setForceFocus(enabled),

    recordFatalRendererError: (error) => window.api.recordFatalRendererError(error),
    recordProductMetric: (name) => window.api.recordProductMetric(name),
    productAnalytics: {
      enabled: () => window.api.getProductAnalyticsEnabled(),
      setEnabled: (enabled) => window.api.setProductAnalyticsEnabled(enabled),
    },
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://swift-scale.com/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("swiftcoder.global.dat").getItem("language")
