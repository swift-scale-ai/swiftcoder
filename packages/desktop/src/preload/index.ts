...
const api: ElectronAPI = {
  ...
  swiftScaleAuth: {
    status: () => ipcRenderer.invoke("swiftscale-auth-status"),
    login: () => ipcRenderer.invoke("swiftscale-auth-login"),
    logout: () => ipcRenderer.invoke("swiftscale-auth-logout"),
    entitlements: (refresh = false) => ipcRenderer.invoke("swiftscale-entitlements", refresh),
    subscribe: (cb) => {
      const handler = (_: unknown, status: Parameters<typeof cb>[0]) => cb(status)
      ipcRenderer.on("swiftscale-auth-changed", handler)
      return () => ipcRenderer.removeListener("swiftscale-auth-changed", handler)
    },
  },
  ...
  recordProductMetric: (name) => ipcRenderer.invoke("record-product-metric", name),
  getProductAnalyticsEnabled: () => ipcRenderer.invoke("get-product-analytics-enabled"),
  setProductAnalyticsEnabled: (enabled) => ipcRenderer.invoke("set-product-analytics-enabled", enabled),
