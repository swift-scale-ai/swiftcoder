...
export const SWIFTCODER_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const SWIFTCODER_MODE_KEY = "swiftcoder.mode"

export const SwiftCoderKeymapProvider = KeymapProvider
export const useSwiftCoderKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type SwiftCoderModeStack = ReturnType<typeof createSwiftCoderModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, SwiftCoderModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createSwiftCoderModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(SWIFTCODER_MODE_KEY, SWIFTCODER_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(SWIFTCODER_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(SWIFTCODER_MODE_KEY, stack.at(-1)?.mode ?? SWIFTCODER_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? SWIFTCODER_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(SWIFTCODER_MODE_KEY, undefined)
...
export function useSwiftCoderModeStack() {
  return getSwiftCoderModeStack(useSwiftCoderKeymap())
}

export function getSwiftCoderModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("SwiftCoder mode stack is not registered for this keymap")
  return value
}
...
export function registerSwiftCoderKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createSwiftCoderModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useSwiftCoderKeymap()
