import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@swiftscale/plugin/tui"
...
import type { useSwiftCoderKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useSwiftCoderKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getSwiftCoderModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getSwiftCoderModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
