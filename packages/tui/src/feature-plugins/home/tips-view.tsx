import type { TuiPluginApi } from "@swiftscale/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("swiftcoder.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public swift-scale.com link",
  ...
  "Create {highlight}swiftcoder.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.swiftcoder/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.swiftcoder/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.swiftcoder/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.swiftcoder/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent SwiftCoder from reading sensitive files",
  "Use {highlight}swiftcoder run{/highlight} for non-interactive scripting",
  "Use {highlight}swiftcoder --continue{/highlight} to resume the last session",
  "Use {highlight}swiftcoder run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}swiftcoder serve{/highlight} for headless API access to SwiftCoder",
  "Use {highlight}swiftcoder run --attach{/highlight} to connect to a running server",
  "Run {highlight}swiftcoder upgrade{/highlight} to update to the latest version",
  "Run {highlight}swiftcoder auth list{/highlight} to see all configured providers",
  "Run {highlight}swiftcoder agent create{/highlight} for guided agent creation",
  "Use {highlight}/opencode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}swiftcoder github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/opencode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.swiftcoder/themes/{/highlight} directory",
  ...
  "Run {highlight}swiftcoder debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm registry.swift-scale.com/swiftcoder{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with SwiftCoder for curated, tested models",
