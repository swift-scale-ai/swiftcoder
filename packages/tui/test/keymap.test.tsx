...
import { getSwiftCoderModeStack, SWIFTCODER_BASE_MODE, SwiftCoderKeymapProvider, registerSwiftCoderKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerSwiftCoderKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when swiftcoder mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getSwiftCoderModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getSwiftCoderModeStack(keymap).push("autocomplete")
