...
Private generation target for clients derived directly from SwiftCoder's authoritative Effect `HttpApi`.
...
```ts
import { AbsolutePath, Location, SwiftCoder, Prompt } from "@opencode-ai/client/effect"

const client = yield * SwiftCoder.make({ baseUrl: "https://opencode.example" })
yield *
